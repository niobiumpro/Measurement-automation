...
class Agilent_DSO(Instrument):
	...
	def __init__(self, name, address):
		...
		self._visainstrument = visa.instrument(self._address, timeout=25)
		
		# number of channels
		self._numchs = 4
		
		# initialize virtual instrument
		self.add_parameter("test", flags = Instrument.FLAG_GETSET, 
	...
	def acqmode_averaging(self, count):
		self._visainstrument.write(':ACQ:TYPE AVERage')
		self._visainstrument.write(':ACQ:COUNT {0}'.format(count))
	...
	def do_set_yoffset(self, value, channel):
		return self._visainstrument.write(':CHAN%d:OFFS %d'%(channel, value))
	...
	def do_set_yrange(self, value, channel):
		return self._visainstrument.write(':CHAN%d:RANG %d'%(channel, value))
	...
	def get_data(self, channel):
		...
		def conv_to_float_if_possible(vals):
			f_vals = []
			for x in vals:
				f_vals.append(float(x))
			return f_vals
		values = conv_to_float_if_possible(values)
		parts = [
		...
		for i in range(len(values)):
			preamble.update({parts[i]:values[i]}) 
		##preamble = dict(parts=values)
		
		
		# retrieve data from device
		#self._visainstrument.write(':WAV:SOUR CHAN%s'%channel)
		#raw_data = (np.array(self._visainstrument.ask_for_values(':WAV:DATA?', 1)) \
		#-preamble["Yreference"])*preamble["Yincrement"]+preamble["Yorigin"] # start,size; format ascii, single, double, big_endian
		#raw_data = self._visainstrument.ask_for_values(':WAV:DATA?', 1)
		#data = np.fromstring(raw_data, np.int16) ###??? not float?
		data = np.array(self._visainstrument.ask_for_values(':WAV:SOUR CHAN%s; :WAV:DATA?'%channel, ascii))
        
		return data[1:], preamble
        
	def get_data_raw(self, channel):
		self._visainstrument.write(':STOP')
		self._visainstrument.write(':WAVeform:FORMat WORD')
		self._visainstrument.write(':WAVeform:POINts:MODE RAW')
		self._visainstrument.write(':WAVeform:UNSigned OFF')
		#max_point_no = 1000000
		max_point_no = float(self._visainstrument.ask(':WAVeform:POINts? MAXimum'))
		#print 'Maximum point number available: {0}'.format(max_point_no)
		self._visainstrument.write(':WAVeform:POINts {0}'.format(max_point_no))
		    
		self._visainstrument.write(':DIGitize CHANnel%s'%channel)
		#data_raw = struct.unpack('{0}h'.format(max_point_no),self._visainstrument.ask(':WAV:DATA?'))
		data_raw = self._visainstrument.ask(':WAV:DATA?')
		data = struct.unpack('{0}h'.format(len(data_raw[10:])/2), data_raw[10:])
        
        #collecting preamble
		preamble_raw = self._visainstrument.ask(':WAVeform:PREamble?').split(',')
		def conv_to_float_if_possible(vals):
			f_vals = []
			for x in vals:
				f_vals.append(float(x))
			return f_vals
		preamble_floats = conv_to_float_if_possible(preamble_raw)
		parts = [
			'format', 'type', 'points', 'count', 
			'Xincrement', 'Xorigin', 'Xreference',
			'Yincrement', 'Yorigin', 'Yreference', 'coupling', 
		...
		preamble = {}
		for i in range(len(preamble_floats)):
			preamble.update({parts[i]:preamble_floats[i]}) 
        
		data = np.asarray(data)*preamble['Yincrement']
