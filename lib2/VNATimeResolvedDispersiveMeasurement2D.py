...
class VNATimeResolvedDispersiveMeasurement2D(VNATimeResolvedDispersiveMeasurement):
    ...
    def __init__(self, name, sample_name, vna_name, ro_awg_name, q_awg_name,
        ...
        super().__init__(name, sample_name, vna_name,
                ro_awg_name, q_awg_name, q_lo_name, plot_update_interval=5)

    def set_fixed_parameters(self, vna_parameters, ro_awg_parameters,
                q_awg_parameters, pulse_sequence_parameters):
...
class VNATimeResolvedDispersiveMeasurement2DResult(
    ...
    def _plot(self, axes, caxes):
        ...
        X, Y = self._prepare_data_for_plot(data)
        ...
        for idx, name in enumerate(self._data_formats.keys()):
            ...
            Z = self._data_formats[name][0](data["data"])
            max_Z = max(Z[Z!=0])
            min_Z = min(Z[Z!=0])
