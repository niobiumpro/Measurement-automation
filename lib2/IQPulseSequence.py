...
class PulseBuilder():
    ...
    def add_sine_pulse(self, duration, phase = 0, time_offset = None):
        """
        ...
        if time_offset is None:
            phase+=(self._pulse_seq_I.total_points()-1)*\
                    self._waveform_resolution*2*pi*frequency/1e9
        else:
            rounded_time_offset =\
             round(time_offset/self._waveform_resolution)*\
                                self._waveform_resolution
            phase += 2*pi*frequency/1e9*rounded_time_offset
    ...
    def build_dispersive_APE_sequences(exc_pb, ro_pb,
        ...
        ro_pb.add_zero_pulse(2*half_pi_pulse_duration+5+\
              pseudo_I_pulses_count*(10+2*half_pi_pulse_duration))\
             .add_dc_pulse(readout_duration)\
             .add_zero_pulse(100)
