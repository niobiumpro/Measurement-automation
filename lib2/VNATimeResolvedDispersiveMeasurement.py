...
class VNATimeResolvedDispersiveMeasurement(Measurement):
    ...
    # TODO check carefully. All single device functions should be deleted?
        ...
        dev_params['vna']["trigger_type"] = "single"
        freq_limits = dev_params['vna']["freq_limits"]
        ...
        if detect_resonator and freq_limits[0]!=freq_limits[1]:
            ...
            res_freq = self._detect_resonator(dev_params['vna'],
                                              dev_params['ro_awg']["calibration"],
                                              dev_params['q_awg'][0]["calibration"],
                                              q_z_cal, plot_resonator_fit=plot_resonator_fit)
            dev_params['vna']["freq_limits"] = (res_freq, res_freq)
    ...
    def _detect_resonator(self, vna_parameters, ro_calibration, q_calibration,
        ...
        self._q_lo_list[0].set_output_state("OFF")
        print("Detecting a resonator within provided frequency range of the VNA %s\
                    "%(str(vna_parameters["freq_limits"])))
        self._vna_list[0].set_nop(201)
        self._vna_list[0].set_freq_limits(*vna_parameters["freq_limits"])
        self._vna_list[0].set_power(vna_parameters["power"])
        self._vna_list[0].set_bandwidth(vna_parameters["bandwidth"]*10)
        self._vna_list[0].set_averages(vna_parameters["averages"])
        ...
        self._ro_awg_list[0].output_pulse_sequence(ro_pb\
                    .add_dc_pulse(ro_duration).add_zero_until(rep_period).build())
        self._q_awg_list[0].output_pulse_sequence(q_pb.add_zero_until(rep_period).build())
        if self._q_z_awg_list[0] is not None:
            q_z_pb = PulseBuilder(q_z_calibration)
            self._q_z_awg_list[0].output_pulse_sequence(q_z_pb.add_zero_until(rep_period).build())
        ...
        self._q_lo_list[0].set_output_state("ON")
        return res_freq

    def _output_pulse_sequence(self):

        q_pbs = [q_awg.get_pulse_builder() for q_awg in self._q_awg_list]
        ro_pbs = [ro_awg.get_pulse_builder() for ro_awg in self._ro_awg_list]
        q_z_pbs = [q_z_awg.get_pulse_builder() for q_z_awg in self._q_z_awg_list]
        pbs = {'q_pbs':q_pbs,
               'ro_pbs':ro_pbs,
               'q_z_pbs':q_z_pbs}
        ...
        for (seq, dev) in zip(seqs['q_seqs'], self._q_awg_list):
            dev.output_pulse_sequence(seq)
        for (seq, dev) in zip(seqs['ro_seqs'], self._ro_awg_list):
            dev.output_pulse_sequence(seq, async=True)
        if 'q_z_seqs' in seqs.keys():
            for (seq, dev) in zip(seqs['q_z_seqs'], self._q_z_awg_list):
                dev.output_pulse_sequence(seq, async=True)
