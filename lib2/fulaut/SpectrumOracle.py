...
class SpectrumOracle():
    ...
    def launch(self, plot=False):
        ...
        d_slice = slice(candidate_params[2], candidate_params[2] + 0.1, 1)
    ...
    def _extract_data(self, plot=False):
        ...
        for idx in range(len(self._parameter_values)):
            ...
            bright_extrema = bright_extrema[row[bright_extrema] > 1 * self._threshold]
        ...
        for point in points:
            ...
            very_close_y_indices = where(abs(self._points[:, 1] - point[1]) < 0.001)[0]
            if len(very_close_y_indices) > 20:
                self._points = delete(self._points, very_close_y_indices, 0).reshape(-1, 2)
