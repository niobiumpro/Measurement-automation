...
def save_IQMX_calibration(iqmx_calibration):
	...
	try:
		...
		new_cal_data={iqmx_calibration.get_mixer_parameters()["iq_attenuation"]:{frozenset(iqmx_calibration.get_radiation_parameters().items()):iqmx_calibration}}
...
def save_measurement(measurement, filename, plot_amps_kwargs={}, plot_phas_kwargs ={}, plot_kwargs={}):
	...
	for fig in pl.plot_measurement(measurement, plot_amps_kwargs, plot_phas_kwargs, **plot_kwargs):
		fig.savefig(directory+subdirectory+"\\"+filename+" "+fig.get_axes()[0].get_title()+".png", dpi=600, bbox_inches="tight")
